...
For local development with MySQL and RabbitMQ running in Docker, keep connection values in `.env`. The scripts also support `.env.local` if you prefer that name later.

Start or repair the local infrastructure with:

```powershell
.\scripts\setup-local-infra.cmd
```
...
Spring also imports `.env` and `.env.local` automatically, so starting the app from an IDE or plain Maven still uses the local connection values.

Local backend database settings:
...
JWT_SECRET=local-compose-secret-must-be-at-least-32-bytes-long
SERVER_SSL_ENABLED=false
```

With `SERVER_SSL_ENABLED=false`, the local backend listens on plain HTTP:

```text
http://127.0.0.1:8080
```

Local RabbitMQ settings:

```properties
RABBITMQ_HOST=localhost
RABBITMQ_PORT=5672
RABBITMQ_USERNAME=bar_scan
RABBITMQ_PASSWORD=bar_scan
...
Flyway also seeds one development admin user for local Docker Compose usage:
...
Docker Compose also enables a local bootstrapper that upserts this same company/admin on backend startup. That means the `admin` / `2` login is restored even if an older local MySQL volume already existed before the admin migration was added.

Use this admin to log in locally and call `/auth/register` for additional users. Change or remove the seed/bootstrap settings before using the stack outside local development.
